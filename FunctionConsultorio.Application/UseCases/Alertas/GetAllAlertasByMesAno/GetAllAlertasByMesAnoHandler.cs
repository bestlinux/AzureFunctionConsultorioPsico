...
namespace FunctionConsultorio.Application.UseCases.Alertas.GetAllAlertasByMesAno
{
        ...
        public async Task<IReadOnlyCollection<GetAllAlertasByMesAnoResponse>> Handle(GetAllAlertasByMesAnoRequest request, CancellationToken cancellationToken)
        {
            try
            {
                ...
                foreach (var paciente in pacientes)
                {
                    var entityAlerta = new Alerta
                    {
                        Descricao = string.Concat("Paciente ", paciente?.Nome, " faz aniversário no dia ", paciente?.DataNascimento.Value.Day, " deste mês ! Não esqueça os parabéns !"),
                        Categoria = 1
                    };
                    alertas.Add(entityAlerta);
                }
